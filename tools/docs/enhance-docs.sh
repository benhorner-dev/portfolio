...
main() {
    ...
    if [ -z "$ts_files" ]; then
        ...
        if [ -f "$ENHANCED_FILES_OUTPUT" ]; then
            local total_enhanced
            total_enhanced=$(wc -l < "$ENHANCED_FILES_OUTPUT" 2>/dev/null || echo "0")
            print_info "📝 Enhanced files list saved to: $ENHANCED_FILES_OUTPUT"
            print_info "📊 Total files enhanced: $total_enhanced"

            if [ "$total_enhanced" -gt 0 ]; then
                print_info "📋 Enhanced files:"
                while IFS= read -r enhanced_file; do
                    print_info "  - $enhanced_file"
                done < "$ENHANCED_FILES_OUTPUT"
            fi
        fi
    fi
